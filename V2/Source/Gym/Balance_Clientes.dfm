object FAjuste_Balances_Clientes: TFAjuste_Balances_Clientes
  Left = 194
  Top = 47
  BorderStyle = bsToolWindow
  Caption = 'Actualizacion Balance de Clientes'
  ClientHeight = 576
  ClientWidth = 765
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Detalles_Compra: TAdvPageControl
    Left = 4
    Top = 8
    Width = 757
    Height = 489
    ActivePage = Page_Detalles
    ActiveFont.Charset = DEFAULT_CHARSET
    ActiveFont.Color = clWindowText
    ActiveFont.Height = -11
    ActiveFont.Name = 'Tahoma'
    ActiveFont.Style = []
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Images = Modulo.SystemImages
    ParentFont = False
    TabBorderColor = clBlack
    TabSheetBorderColor = clBlack
    TabBackGroundColor = clBtnFace
    TabBackGround.Data = {
      
      98F4}
    TabMargin.LeftMargin = 15
    TabMargin.RightMargin = 0
    TabOverlap = 0
    TabSplitLine = True
    RoundEdges = True
    Version = '2.0.0.1'
    PersistPagesState.Location = plRegistry
    PersistPagesState.Enabled = False
    TabHeight = 19
    TabOrder = 0
    TabStop = False
    TabWidth = 250
    object Page_Detalles: TAdvTabSheet
      Caption = 'Detalles de Clientes'
      Color = clBtnFace
      ColorTo = clNone
      ImageIndex = 31
      TabGradientDirection = gdHorizontal
      TabColor = clBtnFace
      TabColorTo = clOlive
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Label1: TLabel
        Left = 3
        Top = 271
        Width = 68
        Height = 13
        Caption = 'Buscar Cliente'
      end
      object Label2: TLabel
        Left = 3
        Top = 306
        Width = 97
        Height = 18
        Caption = 'Balance Real'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 3
        Top = 333
        Width = 78
        Height = 18
        Caption = 'Diferencia'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label4: TLabel
        Left = 240
        Top = 443
        Width = 144
        Height = 13
        Caption = 'Detalles de Factura Pendiente'
      end
      object Buscar_Producto: TwwIncrementalSearch
        Left = 102
        Top = 271
        Width = 644
        Height = 21
        DataSource = Modulo.dListado_Clientes
        SearchField = 'NombreCompleto'
        TabOrder = 1
      end
      object grdClientes: TwwDBGrid
        Left = 0
        Top = 3
        Width = 746
        Height = 262
        ControlInfoInDataset = False
        ControlType.Strings = (
          'Codigo;CustomEdit;Codigos;F'
          'Descripcion;CustomEdit;wwDBLookupCombo1;F'
          'Mes;CustomEdit;seMes;F'
          'Ano;CustomEdit;seAnio;F')
        Selected.Strings = (
          'Estado'#9'1'#9'*'#9'F'
          'Cedula_Rnc_Pasaporte'#9'25'#9'C'#233'dula/Rnc/Registro'#9'F'
          'NombreCompleto'#9'54'#9'Nombre del Cliente'#9'F'
          'Numero_Telefono_1'#9'20'#9'Telefono Principal'#9'F'
          'Balance_Actual'#9'13'#9'Balance Actual'#9'F')
        IniAttributes.Enabled = True
        IniAttributes.FileName = 'Balances.Grd'
        IniAttributes.SectionName = 'Balances_Clientes'
        IniAttributes.Delimiter = ';;'
        TitleColor = 13428432
        OnRowChanged = grdClientesRowChanged
        FixedCols = 0
        ShowHorzScrollBar = True
        DataSource = Modulo.dListado_Clientes
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyOptions = [dgEnterToTab, dgAllowDelete, dgAllowInsert]
        Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap, dgShowCellHint]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleAlignment = taCenter
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = [fsBold]
        TitleLines = 2
        TitleButtons = False
        UseTFields = False
        OnCalcCellColors = grdClientesCalcCellColors
        OnExit = DetallesExit
        PadColumnStyle = pcsPlain
        PaintOptions.AlternatingRowColor = 9948159
        object wwIButton1: TwwIButton
          Left = 0
          Top = 0
          Width = 13
          Height = 22
          AllowAllUp = True
          Flat = True
        end
      end
      object Balance: TEdit
        Left = 102
        Top = 303
        Width = 138
        Height = 26
        BiDiMode = bdRightToLeft
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentFont = False
        TabOrder = 2
        Text = '0.00'
        OnExit = BalanceExit
      end
      object Descripcion_Pendientes: TMemo
        Left = 240
        Top = 303
        Width = 506
        Height = 138
        Lines.Strings = (
          'Descripcion de las Facturas Pendientes')
        TabOrder = 3
      end
      object Diferencia: TEdit
        Left = 102
        Top = 330
        Width = 138
        Height = 26
        TabStop = False
        BiDiMode = bdRightToLeft
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentColor = True
        ParentFont = False
        ReadOnly = True
        TabOrder = 4
        Text = '0.00'
      end
    end
  end
  object bt_Aceptar: TsuiButton
    Left = 472
    Top = 503
    Width = 91
    Height = 28
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Caption = 'Guardar'
    AutoSize = False
    ParentFont = False
    UIStyle = DeepBlue
    Action = Guardar
    TabOrder = 1
    Transparent = True
    ModalResult = 0
    FocusedRectMargin = 2
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
    Layout = blGlyphLeft
    Spacing = 4
    MouseContinuouslyDownInterval = 100
    ResHandle = 0
  end
  object bt_Cancelar: TsuiButton
    Left = 666
    Top = 503
    Width = 91
    Height = 28
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Caption = 'Salir'
    AutoSize = False
    ParentFont = False
    UIStyle = DeepBlue
    Action = Salir
    TabOrder = 2
    Transparent = True
    ModalResult = 0
    FocusedRectMargin = 2
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
    Layout = blGlyphLeft
    Spacing = 4
    MouseContinuouslyDownInterval = 100
    ResHandle = 0
  end
  object suiButton1: TsuiButton
    Left = 569
    Top = 503
    Width = 91
    Height = 28
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Caption = 'Imprimir'
    AutoSize = False
    ParentFont = False
    UIStyle = DeepBlue
    Action = Imprimir
    TabOrder = 3
    Transparent = True
    ModalResult = 0
    FocusedRectMargin = 2
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
    Layout = blGlyphLeft
    Spacing = 4
    MouseContinuouslyDownInterval = 100
    ResHandle = 0
  end
  object suiButton2: TsuiButton
    Left = 8
    Top = 503
    Width = 177
    Height = 28
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Caption = 'Imprimir Plantilla de Balances'
    AutoSize = False
    ParentFont = False
    UIStyle = DeepBlue
    Action = Imprimir_Plantilla_Conteo
    TabOrder = 4
    Transparent = True
    ModalResult = 0
    FocusedRectMargin = 2
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
    Layout = blGlyphLeft
    Spacing = 4
    MouseContinuouslyDownInterval = 100
    ResHandle = 0
  end
  object Opciones: TActionManager
    Images = Modulo.SystemImages
    Left = 256
    Top = 200
    StyleName = 'XP Style'
    object Salir: TAction
      Category = 'Acciones y Desplazamiento'
      Caption = 'Salir'
      ImageIndex = 10
      ShortCut = 27
      OnExecute = SalirExecute
    end
    object Guardar: TAction
      Category = 'Opciones'
      Caption = 'Guardar'
      ImageIndex = 0
      ShortCut = 16455
      OnExecute = GuardarExecute
    end
    object Nuevo: TAction
      Category = 'Opciones'
      Caption = 'Nuevo'
      ImageIndex = 1
      ShortCut = 16462
    end
    object Imprimir: TAction
      Category = 'Opciones'
      Caption = 'Imprimir'
      ImageIndex = 5
      OnExecute = ImprimirExecute
    end
    object Imprimir_Plantilla_Conteo: TAction
      Category = 'Opciones'
      Caption = 'Imprimir Plantilla de Conteo'
      ImageIndex = 7
      OnExecute = Imprimir_Plantilla_ConteoExecute
    end
    object Imprimir_Plantilla_Verificacion: TAction
      Category = 'Opciones'
      Caption = 'Imprimir Plantilla de Verificacion'
      ImageIndex = 7
      OnExecute = Imprimir_Plantilla_VerificacionExecute
    end
  end
end
