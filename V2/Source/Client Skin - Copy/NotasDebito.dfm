object FNotas_Debito: TFNotas_Debito
  Left = 9
  Top = -5
  BorderStyle = bsToolWindow
  Caption = 'Notas de D'#233'bito'
  ClientHeight = 578
  ClientWidth = 794
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = Opciones_Miscelaneas
  Position = poScreenCenter
  ShowHint = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel_Facturas: TAdvPageControl
    Left = 306
    Top = 1
    Width = 483
    Height = 194
    ActivePage = Pg_Facturas
    ActiveFont.Charset = DEFAULT_CHARSET
    ActiveFont.Color = clWindowText
    ActiveFont.Height = -11
    ActiveFont.Name = 'Tahoma'
    ActiveFont.Style = []
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    HotTrack = True
    Images = Modulo.SystemImages
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabBorderColor = clBlack
    TabSheetBorderColor = clBlack
    TabBackGroundColor = clBtnFace
    TabBackGround.Data = {
      
      98F4}
    TabMargin.LeftMargin = 15
    TabMargin.RightMargin = 0
    TabOverlap = 0
    TabSplitLine = True
    RoundEdges = True
    Version = '1.6.0.0'
    TabHeight = 19
    TabOrder = 0
    TabStop = False
    TabWidth = 150
    object Pg_Facturas: TAdvTabSheet
      Caption = 'Facturas'
      Color = clBtnFace
      ColorTo = clNone
      ImageIndex = 58
      TabGradientDirection = gdHorizontal
      TabColor = clBlack
      TabColorTo = clOlive
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Facturas_Cliente: TwwDBGrid
        Left = 0
        Top = 0
        Width = 475
        Height = 165
        Selected.Strings = (
          'Numero'#9'12'#9'# Factura'#9'F'
          'Condicion'#9'8'#9'Cond.'#9'F'
          'Fecha'#9'11'#9'Fecha'#9'F'
          'Inicial'#9'13'#9'Inicial'#9'F'
          'Modalidad'#9'12'#9'Modalidad'#9'F'
          'Pagado'#9'13'#9'Pagado'#9'F'
          'Pendiente'#9'13'#9'Pendiente'#9'F'
          'Vencimiento'#9'11'#9'Vencimiento'#9'F'
          'Periodo_Pagos'#9'25'#9'Periodo'#9'F'
          'Plazo_Pago'#9'10'#9'# Cuotas'#9'F'
          'Status'#9'15'#9'Estatus'#9'F'
          'Tipo_Factura'#9'15'#9'Tipo_Factura'#9'F'
          'TotalNeto'#9'13'#9'Total Neto'#9'F'
          'UltimoPago'#9'9'#9'Ultimo Pago'#9'F')
        IniAttributes.Enabled = True
        IniAttributes.FileName = 'NotasDebito.Grd'
        IniAttributes.SectionName = 'Facturas'
        IniAttributes.Delimiter = ';;'
        TitleColor = 12615680
        FixedCols = 0
        ShowHorzScrollBar = True
        Align = alClient
        BorderStyle = bsNone
        Ctl3D = False
        DataSource = Modulo.dFacturas_Cliente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -9
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 0
        TitleAlignment = taCenter
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clYellow
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        TitleLines = 1
        TitleButtons = False
        UseTFields = False
        OnCalcCellColors = Facturas_ClienteCalcCellColors
        PadColumnStyle = pcsPlain
        PaintOptions.AlternatingRowColor = 15792383
        object Facturas_ClienteIButton: TwwIButton
          Left = 0
          Top = 0
          Width = 13
          Height = 22
          AllowAllUp = True
        end
      end
    end
  end
  object Panel_Botones: TAdvPageControl
    Left = 8
    Top = 384
    Width = 177
    Height = 186
    ActivePage = Pg_Botones
    ActiveFont.Charset = DEFAULT_CHARSET
    ActiveFont.Color = clWindowText
    ActiveFont.Height = -11
    ActiveFont.Name = 'Tahoma'
    ActiveFont.Style = []
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Images = Modulo.SystemImages
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabBorderColor = clBlack
    TabSheetBorderColor = clBlack
    TabBackGroundColor = clBtnFace
    TabBackGround.Data = {
      
      98F4}
    TabMargin.LeftMargin = 15
    TabMargin.RightMargin = 0
    TabOverlap = 0
    TabSplitLine = True
    RoundEdges = True
    Version = '1.6.0.0'
    TabHeight = 19
    TabOrder = 1
    TabStop = False
    TabWidth = 100
    object Pg_Botones: TAdvTabSheet
      Color = clBtnFace
      ColorTo = clNone
      ImageIndex = -1
      TabGradientDirection = gdHorizontal
      TabColor = clBlack
      TabColorTo = clOlive
      object Botones: TActionToolBar
        Left = 0
        Top = 0
        Width = 169
        Height = 157
        ActionManager = Opciones
        Align = alClient
        Caption = 'Botones'
        ColorMap = Modulo.ColorConfig
        EdgeInner = esNone
        Orientation = boTopToBottom
        PersistentHotKeys = True
        Spacing = 0
      end
    end
  end
  object Panel_Debitos: TPanel
    Left = 306
    Top = 432
    Width = 483
    Height = 133
    BevelInner = bvSpace
    BevelOuter = bvLowered
    ParentColor = True
    TabOrder = 2
    Visible = False
    object LValorPago: TLabel
      Left = 4
      Top = 7
      Width = 64
      Height = 13
      Caption = 'Valor Pagado'
      FocusControl = Valor
    end
    object LFechaPago: TLabel
      Left = 4
      Top = 30
      Width = 33
      Height = 13
      Caption = 'Fecha '
    end
    object Label1: TLabel
      Left = 4
      Top = 78
      Width = 46
      Height = 13
      Caption = 'Concepto'
      FocusControl = Concepto
    end
    object Label2: TLabel
      Left = 4
      Top = 53
      Width = 31
      Height = 13
      Caption = 'Vence'
    end
    object Valor: TDBEdit
      Left = 84
      Top = 4
      Width = 121
      Height = 19
      Ctl3D = False
      DataField = 'Monto'
      DataSource = Modulo.dNotas_Debito
      ParentCtl3D = False
      TabOrder = 0
    end
    object Fecha: TwwDBDateTimePicker
      Left = 84
      Top = 26
      Width = 121
      Height = 21
      CalendarAttributes.Font.Charset = DEFAULT_CHARSET
      CalendarAttributes.Font.Color = clWindowText
      CalendarAttributes.Font.Height = -11
      CalendarAttributes.Font.Name = 'MS Sans Serif'
      CalendarAttributes.Font.Style = []
      DataField = 'Fecha'
      DataSource = Modulo.dNotas_Debito
      Epoch = 1950
      ShowButton = True
      TabOrder = 1
    end
    object chk_Imprimir_Recibo: TCheckBox
      Left = 326
      Top = 99
      Width = 149
      Height = 17
      Caption = 'Imprimir &Comprobante'
      Checked = True
      State = cbChecked
      TabOrder = 7
    end
    object Concepto: TDBEdit
      Left = 84
      Top = 74
      Width = 395
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'Concepto'
      DataSource = Modulo.dNotas_Debito
      ParentCtl3D = False
      TabOrder = 4
    end
    object bt_Aplicar_Credito: TsuiButton
      Left = 84
      Top = 96
      Width = 115
      Height = 28
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Caption = 'Aplicar'
      AutoSize = False
      ParentFont = False
      UIStyle = DeepBlue
      Action = Aplicar
      TabOrder = 5
      Transparent = True
      ModalResult = 0
      FocusedRectMargin = 2
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
      Layout = blGlyphLeft
      Spacing = 4
      MouseContinuouslyDownInterval = 100
      ResHandle = 0
    end
    object Bt_Cancelar_Credito: TsuiButton
      Left = 205
      Top = 96
      Width = 115
      Height = 28
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Caption = 'Cancelar'
      AutoSize = False
      ParentFont = False
      UIStyle = DeepBlue
      Action = Cancelar
      TabOrder = 6
      Transparent = True
      ModalResult = 0
      FocusedRectMargin = 2
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
      Layout = blGlyphLeft
      Spacing = 4
      MouseContinuouslyDownInterval = 100
      ResHandle = 0
    end
    object Aplicado_A: TwwRadioGroup
      Left = 211
      Top = 4
      Width = 266
      Height = 51
      DisableThemes = False
      ItemIndex = 0
      Caption = '.'
      Columns = 2
      Ctl3D = True
      Items.Strings = (
        'Aplicado al Capital'
        'Aplicado al Inter'#233's'
        'Aplicado a la Mora'
        'Aplicado a Otros')
      ParentCtl3D = False
      TabOrder = 3
    end
    object Vence: TwwDBDateTimePicker
      Left = 84
      Top = 49
      Width = 121
      Height = 21
      CalendarAttributes.Font.Charset = DEFAULT_CHARSET
      CalendarAttributes.Font.Color = clWindowText
      CalendarAttributes.Font.Height = -11
      CalendarAttributes.Font.Name = 'MS Sans Serif'
      CalendarAttributes.Font.Style = []
      DataField = 'Vence'
      DataSource = Modulo.dNotas_Debito
      Epoch = 1950
      ShowButton = True
      TabOrder = 2
    end
  end
  object chk_Vista_Previa: TCheckBox
    Left = 186
    Top = 370
    Width = 118
    Height = 17
    Caption = 'Vista Previa'
    TabOrder = 3
  end
  object Panel_Notas: TAdvPageControl
    Left = 306
    Top = 199
    Width = 481
    Height = 204
    ActivePage = Pg_Notas
    ActiveFont.Charset = DEFAULT_CHARSET
    ActiveFont.Color = clWindowText
    ActiveFont.Height = -11
    ActiveFont.Name = 'Tahoma'
    ActiveFont.Style = []
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    HotTrack = True
    Images = Modulo.SystemImages
    ParentFont = False
    TabBorderColor = clBlack
    TabSheetBorderColor = clBlack
    TabBackGroundColor = clBtnFace
    TabBackGround.Data = {
      
      98F4}
    TabMargin.LeftMargin = 15
    TabMargin.RightMargin = 0
    TabOverlap = 0
    TabSplitLine = True
    RoundEdges = True
    Version = '1.6.0.0'
    TabHeight = 19
    TabOrder = 4
    TabStop = False
    TabWidth = 250
    object Pg_Notas: TAdvTabSheet
      Caption = 'Detalles Nota de Debito'
      Color = 15263976
      ColorTo = clNone
      ImageIndex = 29
      TabGradientDirection = gdHorizontal
      TabColor = clBlack
      TabColorTo = clOlive
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Notas_Cliente: TwwDBGrid
        Left = 0
        Top = 0
        Width = 473
        Height = 175
        Selected.Strings = (
          'Numero'#9'10'#9'Numero'
          'Fecha'#9'10'#9'Fecha'
          'Total'#9'13'#9'Monto'#9'F'
          'Capital'#9'13'#9'Valor Factura'#9'F'#9'Acreditado'
          'Interes'#9'13'#9'Interes'#9'F'#9'Acreditado'
          'Mora'#9'13'#9'Mora'#9'F'#9'Acreditado'
          'Otros'#9'13'#9'Otros'#9'F'#9'Acreditado'
          'NCF'#9'100'#9'NCF'#9'F'
          'NCF_Modificado'#9'100'#9'NCF_Modificado'#9'F'
          'Concepto'#9'100'#9'Concepto'
          'Realizada_Por'#9'100'#9'Realizada_Por'#9'F'
          'Status'#9'15'#9'Status')
        IniAttributes.Enabled = True
        IniAttributes.FileName = 'NotasDebito.Grd'
        IniAttributes.SectionName = 'Notas'
        IniAttributes.Delimiter = ';;'
        TitleColor = 14145280
        FixedCols = 0
        ShowHorzScrollBar = True
        Align = alClient
        BorderStyle = bsNone
        Ctl3D = False
        DataSource = Modulo.dDetalles_Notas_Debito
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -9
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 0
        TitleAlignment = taCenter
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlack
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        TitleLines = 2
        TitleButtons = False
        UseTFields = False
        OnDblClick = Imprimir_ComprobanteExecute
        PadColumnStyle = pcsPlain
        PaintOptions.AlternatingRowColor = 15792383
        PaintOptions.ActiveRecordColor = cl3DLight
      end
    end
  end
  object Panel_Clientes: TAdvPageControl
    Left = 5
    Top = 1
    Width = 300
    Height = 363
    ActivePage = Pg_Clientes
    ActiveFont.Charset = DEFAULT_CHARSET
    ActiveFont.Color = clWindowText
    ActiveFont.Height = -11
    ActiveFont.Name = 'Tahoma'
    ActiveFont.Style = []
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    HotTrack = True
    Images = Modulo.SystemImages
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabBorderColor = clBlack
    TabSheetBorderColor = clBlack
    TabBackGroundColor = clBtnFace
    TabBackGround.Data = {
      
      98F4}
    TabMargin.LeftMargin = 15
    TabMargin.RightMargin = 0
    TabOverlap = 0
    TabSplitLine = True
    RoundEdges = True
    Version = '1.6.0.0'
    TabHeight = 19
    TabOrder = 5
    TabStop = False
    TabWidth = 150
    object Pg_Clientes: TAdvTabSheet
      Caption = 'Datos del Cliente'
      Color = clBtnFace
      ColorTo = clNone
      ImageIndex = 47
      TabGradientDirection = gdHorizontal
      TabColor = clBlack
      TabColorTo = clOlive
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Recuadro_Foto: TShape
        Left = 177
        Top = 258
        Width = 79
        Height = 74
      end
      object FOTO: TImage
        Left = 179
        Top = 260
        Width = 75
        Height = 70
        Cursor = crHandPoint
        Stretch = True
      end
      object Bt_Buscar_Cliente: TsuiButton
        Left = 9
        Top = 285
        Width = 115
        Height = 28
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Caption = 'Buscar Cliente'
        AutoSize = False
        ParentFont = False
        UIStyle = DeepBlue
        Action = Buscar_Cliente
        TabOrder = 0
        Transparent = True
        ModalResult = 0
        FocusedRectMargin = 2
        Glyph.Data = {
          
          FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
        Layout = blGlyphLeft
        Spacing = 4
        MouseContinuouslyDownInterval = 100
        ResHandle = 0
      end
      object Datos_Cliente: TwwDataInspector
        Left = 0
        Top = 1
        Width = 292
        Height = 253
        DisableThemes = False
        BorderStyle = bsNone
        Ctl3D = False
        CaptionColor = clNone
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -9
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = True
        ParentCtl3D = False
        ParentFont = False
        ScrollBars = ssNone
        TabOrder = 1
        DataSource = Modulo.dClientes
        Items = <
          item
            DataSource = Modulo.dClientes
            DataField = 'NombreCompleto'
            Caption = 'Nombre Completo'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Cedula_Rnc_Pasaporte'
            Caption = 'C'#233'dula/Rnc/Registro'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Direccion'
            Caption = 'Direcci'#243'n'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Sector'
            Caption = 'Sector'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Ciudad'
            Caption = 'Ciudad'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Pais'
            Caption = 'Pa'#237's'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Numero_Telefono_1'
            Caption = 'Tel'#233'fono Principal'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Numero_Telefono_2'
            Caption = 'Tel'#233'fono Secundario'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Tipo_Cliente'
            Caption = 'Tipo de Cliente'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'mail'
            Caption = 'mail'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'TerminosFacturacion'
            Caption = 'Terminos de Facturaci'#243'n'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Condicion_Cliente'
            Caption = 'Condici'#243'n'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Descuento_Asignado'
            Caption = 'Descuento Asignado'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Ultima_Factura'
            Caption = #218'ltima Factura'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Fecha_Ultima_Factura'
            Caption = 'Fecha de '#218'ltima Factura'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Limite_Credito'
            Caption = 'L'#237'mite de Cr'#233'dito'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Balance_Actual'
            Caption = 'Balance Actual'
            WordWrap = False
          end
          item
            DataSource = Modulo.dClientes
            DataField = 'Status'
            Caption = 'Estatus'
            WordWrap = False
          end>
        CaptionWidth = 129
        Options = [ovColumnResize, ovRowResize, ovEnterToTab, ovCenterCaptionVert]
        PaintOptions.AlternatingRowRegions = [arrDataColumns, arrActiveDataColumn]
        PaintOptions.AlternatingRowColor = 14217212
        CaptionFont.Charset = ANSI_CHARSET
        CaptionFont.Color = clWindowText
        CaptionFont.Height = -9
        CaptionFont.Name = 'Tahoma'
        CaptionFont.Style = [fsBold]
        LineStyleCaption = ovNoLines
        LineStyleData = ovNoLines
        ReadOnly = True
        CaptionIndent = 3
      end
    end
  end
  object Opciones: TActionManager
    ActionBars = <
      item
        Items = <
          item
            Action = Pagar
            ImageIndex = 29
            ShortCut = 16464
          end
          item
            Action = Imprimir_Comprobante
            ImageIndex = 5
            ShortCut = 16464
          end
          item
            Action = Estado_Cuentas
            ImageIndex = 15
            ShortCut = 16453
          end
          item
            Caption = '-'
          end
          item
            Action = Salir
            Caption = '&Salir'
            ImageIndex = 10
            ShortCut = 16467
          end>
        ActionBar = Botones
      end>
    Images = Modulo.SystemImages
    Left = 200
    Top = 128
    StyleName = 'XP Style'
    object Salir: TAction
      Category = 'Opciones'
      Caption = 'Salir'
      Hint = 'Salir de Esta Pantalla'
      ImageIndex = 10
      ShortCut = 16467
      OnExecute = SalirExecute
    end
    object Buscar_Cliente: TAction
      Category = 'Opciones'
      Caption = 'Buscar Cliente'
      Hint = 'Buscar Cliente'
      ImageIndex = 2
      ShortCut = 16450
      OnExecute = Buscar_ClienteExecute
    end
    object Pagar: TAction
      Category = 'Opciones'
      Caption = 'Debitar'
      Hint = 'Aplicar una Nota de Debito Pr'#233'stamo seleccionado'
      ImageIndex = 29
      ShortCut = 16464
      OnExecute = PagarExecute
    end
    object Estado_Cuentas: TAction
      Category = 'Opciones'
      Caption = 'Estado de Cuentas'
      Hint = 
        'Imprime el Estado de Cuentas del Cliente Actualmente Seleccionad' +
        'o'
      ImageIndex = 15
      ShortCut = 16453
      OnExecute = Estado_CuentasExecute
    end
    object Imprimir_Comprobante: TAction
      Category = 'Opciones'
      Caption = 'Imprimir Comprobante'
      Hint = 'Imprime el Comprobante de Nota de Cr'#233'dito'
      ImageIndex = 5
      ShortCut = 16464
      OnExecute = Imprimir_ComprobanteExecute
    end
    object Aplicar: TAction
      Category = 'Opciones'
      Caption = 'Aplicar'
      Hint = 'Aplicar el Valor Acreditado'
      ImageIndex = 16
      ShortCut = 16397
      OnExecute = AplicarExecute
    end
    object Cancelar: TAction
      Category = 'Opciones'
      Caption = 'Cancelar'
      Hint = 'Cancelar la Operaci'#243'n Actual'
      ImageIndex = 26
      ShortCut = 27
      OnExecute = CancelarExecute
    end
    object Imprimir_Ficha_Prestamo: TAction
      Category = 'Opciones'
      Caption = 'Imprimir Ficha de Pr'#233'stamo'
      ImageIndex = 5
      ShortCut = 16454
      OnExecute = Imprimir_Ficha_PrestamoExecute
    end
    object Configuracion_Comprobante_Nota: TAction
      Category = 'Opciones'
      Caption = 'Configuraci'#243'n del Comprobante de Nota'
      ImageIndex = 68
      ShortCut = 49219
      OnExecute = Configuracion_Comprobante_NotaExecute
    end
    object Configuracion_Ficha_Prestamo: TAction
      Category = 'Opciones'
      Caption = 'Configuracion de Ficha Prestamo'
      ImageIndex = 68
      ShortCut = 49222
      OnExecute = Configuracion_Ficha_PrestamoExecute
    end
  end
  object Opciones_Miscelaneas: TsuiPopupMenu
    Images = Modulo.SystemImages
    OwnerDraw = True
    UIStyle = DeepBlue
    MenuItemHeight = 26
    SeparatorHeight = 5
    BarWidth = 26
    BarColor = 13554646
    BarToColor = 13554646
    Color = clWhite
    SeparatorColor = 8684164
    SelectedBorderColor = 10504772
    SelectedColor = 10504772
    SelectedFontColor = clWhite
    FontColor = clBlack
    BorderColor = clBlack
    FlatMenu = False
    FontName = 'MS Sans Serif'
    FontSize = 8
    FontCharset = DEFAULT_CHARSET
    UseSystemFont = True
    Left = 200
    Top = 184
    object Pop_Imprimir_Factura: TMenuItem
      Action = Imprimir_Ficha_Prestamo
    end
    object Pop_Configuracion_Facturas: TMenuItem
      Action = Configuracion_Ficha_Prestamo
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object Pop_Configuracion_Nota_Credito: TMenuItem
      Action = Configuracion_Comprobante_Nota
    end
  end
end
