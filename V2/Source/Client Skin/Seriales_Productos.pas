...
uses
  ...
  ActnList, XPStyleActnCtrls, ActnMan, Mask, DBCtrls, wwdbedit, Wwdotdot,
  Wwdbcomb;
...
type
  TFSeriales_Productos = class(TForm)
    ...
    procedure ModalidadesKeyPress(Sender: TObject; var Key: Char);
  private
...
procedure TFSeriales_Productos.ModalidadesKeyPress(Sender: TObject;
  var Key: Char);
begin
  If Key = #13 then
    Begin
      If Not(Modulo.tSeriales_Compra.State In DsEditModes) Then Modulo.tSeriales_Compra.Edit;
      Modulo.tSeriales_CompraModalidad_Servicio.Value := Modalidades.Text;
