...
uses
  ...
  Dialogs, StdCtrls, Grids, DB, DBClient, Provider, ExtCtrls, RegKeys, ActnList,
  XPStyleActnCtrls, ActnMan, AdvGlowButton, ToolPanels;

const
  ActionStr: array[TReconcileAction] of string = ('Skip', 'Abort', 'Merge',
    'Correct', 'Cancel', 'Refresh');
  UpdateKindStr: array[TUpdateKind] of string = ('Modificado', 'Insertado',
    'Deleted');
  SCaption = 'Error de Actualización - %s';
  SUnchanged = '<Sin Cambios>';
  SBinary = '(Binary)';
  SAdt = '(ADT)';
  SArray = '(Array)';
  SFieldName = 'Nombre de Campo';
  SOriginal = 'Valor Original';
  SConflict = 'Valor Conflictivo';
  SValue = ' Valor';
  SNoData = '<No Registros>';
  SNew = 'Nuevo';
...
type
  TReconcileErrorForm = class(TForm)
    ...
    Mas_Detalles: TAction;
    Menos_Detalles: TAction;
    gbt_Detalles: TAdvGlowButton;
    ...
    procedure Mas_DetallesExecute(Sender: TObject);
    procedure Menos_DetallesExecute(Sender: TObject);
  private
    FDataSet: TDataSet;
...
procedure TReconcileErrorForm.Mas_DetallesExecute(Sender: TObject);
begin
  gbt_Detalles.Action := Menos_Detalles;
  Self.Height := 470;
end;

procedure TReconcileErrorForm.Menos_DetallesExecute(Sender: TObject);
begin
  gbt_Detalles.Action := Mas_Detalles;
  Self.Height := 190;
end;
...
procedure TReconcileErrorForm.act_AceptarExecute(Sender: TObject);
begin
  ModalResult := MrOK;
end;

procedure TReconcileErrorForm.act_CancelarExecute(Sender: TObject);
begin
...
Initialization
  RegisterClass(TReconcileErrorForm);
