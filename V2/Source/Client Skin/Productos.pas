...
uses
  ...
  DbChart, wwclearbuttongroup, wwradiogroup, RegKeys, Grids, DBGrids, Wwdbigrd,
  Wwdbgrid, TaskDialog;
...
type
  TFProductos = class(TForm)
    ...
    Calculo_Precio_Impuesto: TAction;
    BitBtn1: TBitBtn;
    Siguente: TAction;
    Anterior: TAction;
    BitBtn2: TBitBtn;
    ...
    Label26: TLabel;
    Detalles: TwwDBGrid;
    bt_Anadir_Codigo: TSpeedButton;
    bt_Eliminar_Codigo: TSpeedButton;
    Anadir_Codigo: TAction;
    Eliminar_Codigo: TAction;
    ...
    procedure Calculo_Precio_ImpuestoExecute(Sender: TObject);
    procedure SiguenteExecute(Sender: TObject);
    procedure AnteriorExecute(Sender: TObject);
    procedure PrimeroExecute(Sender: TObject);
    procedure UltimoExecute(Sender: TObject);
    procedure bt_Anadir_CodigoClick(Sender: TObject);
    procedure bt_Eliminar_CodigoClick(Sender: TObject);
    procedure Anadir_CodigoExecute(Sender: TObject);
    procedure Eliminar_CodigoExecute(Sender: TObject);
  private
...
begin
  ...
  Modulo.tProductosMetodo_Calculo_Costo.Value := 'Última Compra';
...
procedure TFProductos.PrimeroExecute(Sender: TObject);
begin
  If Not Modulo.tListadoProductos.Active Then Modulo.tListadoProductos.Open;
  Modulo.tListadoProductos.First;
  Codigo.Text := Trim(Modulo.tListadoProductosCodigo.Value);
  CodigoExit(Codigo);
end;
...
procedure TFProductos.bt_Eliminar_CodigoClick(Sender: TObject);
begin
  Eliminar_Codigo.Execute
end;

procedure TFProductos.Calculo_Precio_ImpuestoExecute(Sender: TObject);
  Var
    P_Final,Impuesto,P_Venta : Currency;
    S : String;
begin
  S := Modulo.tProductosPrecio_Venta_Contado.AsString;
  S := InputBox('Precio Final','Introduzca el Precio Final de Venta', S);
  If Trim(S) = '' Then Exit;
  try
    P_Final := StrToCurr(S);
  Except
    Mensaje('El valor introducido es invalido',self);
    Exit;
  end;
  S := Modulo.tParametrosImpuesto.AsString;
  S := InputBox('Impuesto','Introduzca el Impuesto a Calcular', S);
  If Trim(S) = '' Then Exit;
  try
    Impuesto := StrToCurr(S);
  Except
    Mensaje('El valor introducido es invalido',self);
    Exit;
  end;

  P_Venta := P_Final / (1 + (Impuesto / 100));
  Modulo.tProductos.Edit;
  Modulo.tProductosPaga_Impuesto.Value := True;
  Modulo.tProductosPrecio_Venta_Contado.Value := P_Venta;
  Modulo.tProductosPrecio_Venta_Credito.Value := P_Venta;
  Modulo.tProductosPrecio_Venta_Minimo.Value := P_Venta;
  Modulo.tProductosPrecio_Venta_Por_Mayor_Contado.Value := P_Venta;
  Modulo.tProductosPrecio_Venta_Por_Mayor_Credito.Value := P_Venta;
  Modulo.tProductosPrecio_Unidad.Value := P_Venta;
...
procedure TFProductos.Eliminar_CodigoExecute(Sender: TObject);
begin
  If Modulo.tCodigos_Productos.RecordCount > 0 Then
    Begin
      If AdvAsk('Eliminar Codigo',
                _ConfirmarEliminar,
                'Para eliminar el codigo seleccione Si, para cancelar seleccione NO',
                tiQuestion,
                [cbYes, cbNo],
                Self) = ID_NO Then Exit;

      Modulo.tCodigos_Productos.Delete;
    End;
end;
...
procedure TFProductos.SiguenteExecute(Sender: TObject);
begin
  If Not Modulo.tListadoProductos.Active Then Modulo.tListadoProductos.Open;

  If Trim(Modulo.tProductosCodigo.Value) = '' Then Modulo.tListadoProductos.First
  Else
    begin
      Modulo.tListadoProductos.Locate(Modulo.tProductosCodigo.FieldName,Modulo.tProductosCodigo.Value,[]);
      If Not Modulo.tListadoProductos.Eof Then Modulo.tListadoProductos.Next;
    end;
  Codigo.Text := Trim(Modulo.tListadoProductosCodigo.Value);
  CodigoExit(Codigo);
end;
...
procedure TFProductos.Anadir_CodigoExecute(Sender: TObject);
  Var
...
begin
  ...
  C := UPPERCASE(InputBox('CODIGO DE BARRA','DIGITE EL CODIGO DE BARRA',''));
  If Trim(C) = '' Then
    Begin
      Mensaje('Debe Digitar un Codigo',Self);
      Exit;
    End;
  ...
  Case R Of
     0 : Begin
            D := UPPERCASE(InputBox('DESCRIPCION','DIGITE UNA DESCRIPCION PARA EL CODIGO DE BARRA',''));

            Modulo.tCodigos_Productos.Append;
            Modulo.tCodigos_ProductosProceso.Value := Modulo.tProductosProceso.Value;
            Modulo.tCodigos_ProductosCodigo_Barra.Value := C;
            Modulo.tCodigos_ProductosDescripcion_Barra.Value := D;
            Modulo.tCodigos_Productos.Post;
         End;
     1 : Mensaje ('Ese código está registrado en el producto:'#13+
                  Act_Code + #13 +
                  Act_Description + #13'Por Favor, VERIFIQUE',self);
     2 : Mensaje ('Ocurrió un error al verificar la información:'#13+
...
procedure TFProductos.AnteriorExecute(Sender: TObject);
begin
  If Not Modulo.tListadoProductos.Active Then Modulo.tListadoProductos.Open;
  ...
  Else
    begin
      Modulo.tListadoProductos.Locate(Modulo.tProductosCodigo.FieldName,Modulo.tProductosCodigo.Value,[]);
      If Not Modulo.tListadoProductos.Bof Then Modulo.tListadoProductos.Prior;
    end;
  Codigo.Text := Trim(Modulo.tListadoProductosCodigo.Value);
  CodigoExit(Codigo);
end;
...
procedure TFProductos.bt_Anadir_CodigoClick(Sender: TObject);
begin
  Anadir_Codigo.Execute;
end;
...
procedure TFProductos.UltimoExecute(Sender: TObject);
begin
  If Not Modulo.tListadoProductos.Active Then Modulo.tListadoProductos.Open;
  Modulo.tListadoProductos.Last;
  Codigo.Text := Trim(Modulo.tListadoProductosCodigo.Value);
  CodigoExit(Codigo);
end;
...
//      ReportsAndForms.Cod_Generado.DataPipeline := ReportsAndForms.PipeProductos;
      ReportsAndForms.Codigo_Etiqueta.Data := Modulo.tProductosCod_Generado.Value;
