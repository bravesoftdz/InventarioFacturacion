...
uses
  ...
  XPStyleActnCtrls, ActnMan, ActnColorMaps, Buttons, DB, DBTables, wwrcdpnl,
  ToolWin, ActnCtrls, StdCtrls;
    ...
    Begin
        ...
        While Not pdoxInventario.Eof Do
         ...
         Except
            on E : Exception do
              Begin
                Mensaje('ERROR TRANSFIRIENDO DATOS ' + E.Message);
    ...
    Begin
        ...
        While Not pdoxProveedores.Eof Do
          ...
          Except
            on E: Exception do
              Begin
                Modulo.Mensajes.Text := 'ERROR TRANSFIRIENDO DATOS ' + E.Message;
...
procedure TFImportar.Action1Execute(Sender: TObject);
  Var N : Integer;
begin
  Detalle_Inventario.DataSource := Modulo.dListadoProductos;
  N := 0;
  ...
  While Not Modulo.tListadoProductos.Eof Do
    Try
      Begin
            Inc (N);
            Mensajes.SimpleText :=  IntToStr(N) + ': ' + Modulo.tListadoProductosDescripcion.AsString;
            Modulo.tExistenciaInicial.Append;
            Modulo.tExistenciaInicialProceso.AsString := Modulo.tListadoProductosCodigo.AsString;
            Modulo.tExistenciaInicialFecha.AsDateTime := Date();
            Modulo.tExistenciaInicialCantidad.AsFloat := Modulo.tListadoProductosExistencia.AsFloat;
            Modulo.tExistenciaInicialCodigo.AsString := Modulo.tListadoProductosCodigo.AsString;
            Modulo.tExistenciaInicialCodigo_Real.AsString := Modulo.tListadoProductosCodigo.AsString;
            Modulo.tExistenciaInicialPrecio_Lista.AsCurrency := Modulo.tListadoProductosCosto_Promedio.AsCurrency;
            Modulo.tExistenciaInicialProveedor.AsInteger := 1;
            Modulo.tExistenciaInicialUsuario.AsString := _CurrentUser;
            Modulo.tExistenciaInicialPrecio_Venta_Por_Mayor_Contado.AsCurrency := Modulo.tListadoProductosPrecio_Venta_Contado.AsCurrency;
            Modulo.tExistenciaInicialPrecio_Venta_Por_Mayor_Credito.AsCurrency := Modulo.tListadoProductosPrecio_Venta_Credito.AsCurrency;
            Modulo.tExistenciaInicialPrecio_Venta_Detalle_Contado.AsCurrency := Modulo.tListadoProductosPrecio_Venta_Contado.AsCurrency;
            Modulo.tExistenciaInicialPrecio_Venta_Detalle_Credito.AsCurrency := Modulo.tListadoProductosPrecio_Venta_Credito.AsCurrency;
            Modulo.tExistenciaInicialPrecio_Minimo.AsCurrency := Modulo.tListadoProductosPrecio_Venta_Minimo.AsCurrency;
            Modulo.tExistenciaInicial.Post;
            Modulo.tExistenciaInicial.ApplyUpdates(0);
  ...
  If OpenTable.Execute Then
    Begin
        ...
        While Not pdoxFacturas.Eof Do
          Try
            pdoxClientes.Locate('Codigo',pdoxFacturasCliente.Value,[]);
            ...
            If (pdoxFacturasPendiente.AsCurrency <> 0) Or (Not Pendientes.Checked) Then
              Begin
                ...
                Modulo.tFacturasConcepto_Factura.AsString := 'Factura PENDIENTE # ' +
                ...
                Modulo.tFacturasNombre_Cliente.AsString := pdoxFacturasNombreClientes.AsString;
