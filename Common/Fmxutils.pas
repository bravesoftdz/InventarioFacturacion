...
begin
  ...
  try
    ...
    if Source < 0 then raise EFOpenError.CreateFmt('ERROR', [FileName]);
    try
      Dest := FileCreate(Destination); { create output file; overwrite existing }
      if Dest < 0 then raise EFCreateError.CreateFmt('ERROR', [Destination]);
      try
        repeat
...
function ExecuteFile(const FileName, Params, DefaultDir: string;
  ShowCmd: Integer): THandle;
var
  zFileName, zParams, zDir: array[0..500] of Char;
begin
