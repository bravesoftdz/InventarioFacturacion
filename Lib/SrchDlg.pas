...
{$R *.DFM}
  ...
  Procedure TDlgSearch.OpenDlg(Const TableName    ,
     ...
     Begin
       ...
       Try
         SrchPath := GetCurrentDir;
         Screen.Cursor := crSQLWait;
         SrchCds.Close;
         SrchTable := TableName;

         // IniConfig
         DataGrid.IniAttributes.FileName := SrchPath + '\' + SrchCds.RemoteServer.Owner.Name + TableName + '.srch';
         StartUpIni := TIniFile.Create(SrchPath + '\' + SrchCds.RemoteServer.Owner.Name + TableName + '.strt');
...
begin
  ...
  Except
    Modulo.Mensajes.MessageText := 'No se suede organizar la busqueda utilizando este campo';
    Modulo.Mensajes.PopUp
  end;
