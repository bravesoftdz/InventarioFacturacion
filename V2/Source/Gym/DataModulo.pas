...
uses
  ...
  SUIForm, ComCtrls, OoMisc, AdPort, Graphics, SUIMgr, AdvAlertWindow, Riss_Devices_TLB,
  ActiveX;
...
type
  TModulo = class(TDataModule)
    ...
    tClockLog: TClientDataSet;
    dClockLog: TDataSource;
    tClockLogNumero: TIntegerField;
    tClockLogFecha: TDateField;
    tClockLogDN: TIntegerField;
    tClockLogUsuario: TStringField;
    tClockLogRegistro: TStringField;
    tClockLogAccion: TIntegerField;
    tClockLogRegistrado: TStringField;
    tClockLogCliente: TStringField;
    tClockLogNombre_Cliente: TStringField;
    tClockLogAtraso: TFloatField;
    tClockLogStatus_Balance: TStringField;
    tClockLogStatus: TStringField;
...
{    Function ActualizarExistencia(Const Codigo_Producto : String;
  ...
  public
    { Public declarations }
    procedure SetDeviceInfo(var aDevice: IDevice);
    ...
    function ReadAllLog(agIDevice: IDevice; agDeviceTools: TZd2911Tools; agDeviceConnection: TZd2911DeviceConnection; Const _From,_To : TDate): Integer;
    function ReadNewLog(agIDevice: IDevice; agDeviceTools: TZd2911Tools; agDeviceConnection: TZd2911DeviceConnection; Const _From,_To : TDate): Integer;
  end;
...
uses Utilidades, Registry, DateUtils, Productos,
  ...
  Autorizar, Seriales_Productos, FileSelection, CommonUnt;
...
Procedure TModulo.NotiticationSet(var _StatusBar : TStatusBar);
  ...
  Procedure TModulo.AddRecord(AIRecord: IRecordExt);
    Var
      Q : TClientDataset;
      ID,
      Nombre,
    ...
    Begin
      tClockLog.Close;
      tClockLog.Params.ParamByName('Fecha').AsDate := Enlace.AppServer.ServerDate;
      tClockLog.Open;

      Q := TClientDataSet.Create(Self);
      Q.ConnectionBroker := tClientes.ConnectionBroker;
      Q.ProviderName := 'prReporter';
      Q.CommandText := 'Select * From "Clientes" Where "Numero_Cuenta" = :Cta';
      Q.Params.ParamByName('Cta').AsString := Trim(AIRecord.DIN);
      Q.Open;
      ID := 'N/EXISTE';
      Nombre := 'USUARIO NO REGISTRADO EN EL SISTEMA';
      Status := _NoneString;
      StatusDeuda := 'N/A';
      If Q.RecordCount > 0 Then
        Begin
          ID := Q.FieldByName('Cedula_Rnc_Pasaporte').AsString;
          Nombre := Q.FieldByName('NombreCompleto').AsString;
      ...
      tClockLog.Append;
      tClockLogFecha.AsDateTime := AIRecord.Clock;
      tClockLogNumero.AsInteger := tClockLog.RecordCount + 1;
      tClockLogDN.AsInteger := AIRecord.DN;
      tClockLogUsuario.AsString := AIRecord.DIN;
      tClockLogRegistro.AsString := GetGLogVerify(AIRecord.Verify);
      tClockLogAccion.AsInteger := AIRecord.Action;
      tClockLogRegistrado.AsString := FormatDateTime('dd-mm-yyyy hh:nn:ss', AIRecord.Clock);
      tClockLogCliente.AsString := ID;
      tClockLogNombre_Cliente.AsString := Nombre;
      tClockLogStatus.AsString := Status;
      tClockLogStatus_Balance.AsString := StatusDeuda;
      tClockLogAtraso.AsCurrency := Deuda;
      tClockLog.Post;
      tClockLog.ApplyUpdates(0);
      tClockLog.First;
...
function TModulo.ReadAllLog(agIDevice: IDevice;
                            agDeviceTools: TZd2911Tools;
                            agDeviceConnection: TZd2911DeviceConnection;
                            Const _From,_To : TDate): Integer;
var
...
begin
  ...
  deviceStatus := agDeviceTools.GetBytesByNum(IntToStr(DeviceBusy),NumberType_Int32Bit);

  //Setting device communication status to busy
  bRet := agDeviceConnection.SetPropertyExt_2(DeviceProperty_Enable,extraProperty,agIDevice,deviceStatus);
  if bRet then
  begin
    //Get all attendance logs
    //All attendance logs, the 1st parameter is 0: get all attendance logs,
    //the 2nd parameter isÂ¡Â°removing the new log markÂ¡Â±
    SetArrayValue(extraProperty,0,0);
    SetArrayValue(extraProperty,1,0);
    DecodeDate(_From,year,month,day);
    ...
    bRet := agDeviceConnection.GetPropertyExt_2(DeviceProperty_AttRecords, extraProperty, agIDevice, extraData);
    if bRet then
    begin
      allAtt := agDeviceTools.GetGLogList(extraData);
...
//      lvRecord.Items.Clear;
    ...
    deviceStatus := agDeviceTools.GetBytesByNum(IntToStr(DeviceIdle),NumberType_Int32Bit);
    bRet := agDeviceConnection.SetPropertyExt_2(DeviceProperty_Enable,extraProperty,agIDevice,deviceStatus);
    Result := recordnum;
...
function TModulo.ReadNewLog(agIDevice: IDevice; agDeviceTools: TZd2911Tools;
  agDeviceConnection: TZd2911DeviceConnection; Const _From,_To : TDate): Integer;
var
  deviceStatus: PSafeArray;
  extraData: PSafeArray;
  extraProperty: PSafeArray;
...
begin
  ...
  begin
    ...
    //new attendance logs, the 1st parameter is 1:
    // get new attendance logs, the 2nd parameter isÂ¡Â°removing the new log markÂ¡Â±
    ...
    begin
      ...
      for i := 0 to recordnum - 1 do
      begin
        CurrVol := GetPcVolume(DeviceMaster);
        SetPcVolume(255,DeviceMaster);
        AddRecord(records[i]);
        SetPcVolume(CurrVol,DeviceMaster);
      end;
      Result := recordnum;
      SafeArrayUnaccessData(allAtt);
    end
...
{Function TModulo.ActualizarExistencia(Const Codigo_Producto : String;
  ...
  Begin
 // 28/02/2006 IMGP
 // Esta seccion estÃ¡ ignorada debido a que se modifico el metodos de manejo de existencia
...
procedure TModulo.SetDeviceInfo(var ADevice: IDevice);
begin
  ADevice.DN := _rlj_DN;
  ADevice.Password := _rlj_Password;
  ADevice.ConnectionModel := _rlj_ConnectionModel;
  ADevice.Model := _rlj_Model;
  ADevice.SerialPort := _rlj_SerialPortValue;
  ADevice.Baudrate := _rlj_Baudrate;
  ADevice.IpAddress := _rlj_IpAddress;
  ADevice.IpPort := _rlj_IpPortValue;
  ADevice.CommunicationType := _rlj_CommType;
end;
...
begin
  ...
  If (Trim(tDetalles_SalidaTipo_Precio.Value) = 'Detalle CrÃ©dito') Then tDetalles_SalidaPrecio_Venta.Value  := tProductosPrecio_Venta_Credito.Value;
  ...
  If (Trim(tDetalles_SalidaTipo_Precio.Value) = 'Por Mayor CrÃ©dito') Then tDetalles_SalidaPrecio_Venta.Value  := tProductosPrecio_Venta_Por_Mayor_Credito.Value;
  ...
  If _ChkExistence Then
    If tProductosExistencia.Value <= tProductosMinimo.Value Then
      DisplayMessage(_ScreenCenter,100,
                     'Producto en MÃ­nimo',
                     Format('La existencia de %s es de %s y estÃ¡ por debajo de %s que es su valor MÃ­nimo'#10#13+
                            'Debe realizar una compra de al menos %s para el inventario de este producto/artÃ­culo',
...
begin
  ...
  If Trim(tListadoProductosMetodo_Calculo_Costo.Value) = 'Ãšltima Compra' Then
