...
type
  TFMainMenu = class(TForm)
    ...
    procedure Definir_AlmacenExecute(Sender: TObject);
...
{      BackGroundImage.Width := RegInfo.ReadInteger(_BackgroundWidth);
    ...
    //  Linea.Width := Screen.Width;
      ...
      Opciones_Pantalla.Top := (Screen.Height - Opciones_Pantalla.Height) - 25;
  ...
  Try
    ...
    If DlgSearch.ModalResult = MrOk Then
      Begin
        ...
        ParamSearchKey(Modulo.tClientes,'Cedula_Rnc_Pasaporte',DlgSearch.SrchCds.FieldByName('Cedula_RNC_Pasaporte').AsString);
        Application.CreateForm (TDlgFecha,DlgFecha);
        DlgFecha.Caption := 'Reporte de Mercancía Despachada Por Fecha :';
        DlgFecha.ShowModal;
        If DlgFecha.ModalResult = MROk Then
          Begin
             ...
             Modulo.tSalidas_Cliente.Params.ParamByName('Cliente').AsString := DlgSearch.SrchCds.FieldByName('Cedula_RNC_Pasaporte').AsString;
             Modulo.tSalidas_Cliente.Open;

             Modulo.tMercancia_Desp_Cliente.Params.ParamByName('Cliente').AsString := DlgSearch.SrchCds.FieldByName('Cedula_RNC_Pasaporte').AsString;
      ...
      Begin
        ...
        If DlgFecha.ModalResult = MROk Then
          Begin
             ...
             Modulo.tFacturas_Cliente.Params.ParamByName('Cliente').AsString := DlgSearch.SrchCds.FieldByName('Cedula_RNC_Pasaporte').AsString;
             Modulo.tFacturas_Cliente.Open;

             Modulo.tMercancia_Cliente.Params.ParamByName('Cliente').AsString := DlgSearch.SrchCds.FieldByName('Cedula_RNC_Pasaporte').AsString;
...
procedure TFMainMenu.Definir_AlmacenExecute(Sender: TObject);
begin
  RegInfo.CloseKey;
  RegInfo.OpenKey(_GeneralConfigKey,False);
    Try
      Application.CreateForm(TDlgSearch,DlgSearch);
      DlgSearch.OpenDlg('Almacenes',Modulo.tAlmacenDescripcion.FieldName,'A',wsNormal);
      If DlgSearch.ModalResult = MrOk Then
        Begin
          RegInfo.WriteString(_DetalleFacturaAlmacen,DlgSearch.SrchCds.FieldByName('Codigo').AsString)
        End
    Finally
      DlgSearch.CloseDlg;
